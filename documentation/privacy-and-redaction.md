...
|---|---|---|
| `NarrativeTraceProxy.Create<T>` / `.Create` (raw `DispatchProxy` capture) | **Yes**, via `new ProxyOptions(Redaction: ...)` *(since 0.1.5)* | Threads the policy into every `ValueRenderer.Render` call this interceptor makes (parameters, nested object walks, return values) and into the top-level parameter-name decision alike — see [Configuration Guide §6](guides/configuration.md#6-redaction). Given explicitly, it *replaces* the default decision rather than widening it, so `RedactionPolicy.Disabled` here really disables name-based redaction end to end. |
| DI auto-wrap (`AddNarrativeTracing`) | **Yes**, via `NarrativeTracingDiOptions.Redaction` *(since 0.1.5)* | Threaded into the `ProxyOptions` each wrapped service is constructed with. Falls back to a `RedactionPolicy` registered by `AddNarrativeTrace` (below) in the same container when this call's own `Redaction` is left unset. |
| ASP.NET Core integration (`AddNarrativeTrace`) | **Yes**, via `NarrativeTraceOptions.Redaction` *(since 0.1.5)* | Registers the policy as a `RedactionPolicy` singleton in the same service collection, so `AddNarrativeTracing`'s auto-wrap — a separate package with no reference to this one — can resolve it as a fallback for every service it wraps in the same app. The middleware itself renders no values; this is what lets one policy, configured once, reach every proxy invoked over the course of a request. |
...
| `[Narrated]` / `[OnError]` template placeholders (`NarrationResolver`) | **Yes**, via the same proxy's `ProxyOptions.Redaction` *(since 0.1.5)* | Threaded through from `NarrativeInterceptor` into every `NarrationResolver.Resolve` call, both call sites (entry narration and exception-time error context). Any proxy with `Redaction` left unset still resolves through `RedactionPolicy.Default`. |
...
the offending method, and the fix *(since 0.1.5)*, rather than
...
proxy's own effective policy too *(since 0.1.5)*: a
...
- **A trace's name and a run's name carry no data.** Both are a
  ...
  *(since 0.1.5)*) — never from anything captured — so neither
...
- **A `[NarrativeSummary]` member is trusted code, once you've named it.**
  ...
  value degrades to the typed `<error: TypeName>` placeholder *(since 0.1.5)*, never a
...
- **Adopting an inbound `traceparent` trusts the caller** *(since 0.1.5)*. `NarrativeTraceMiddleware` and `NarrativeTraceConfig`'s
