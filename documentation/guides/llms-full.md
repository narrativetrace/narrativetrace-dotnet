...
| `NarrativeTrace.Proxy` | Runtime | `NarrativeTraceProxy`, `NarrativeInterceptor`, `[Traced]`; reads the Core annotations. Depends on `Runtime` (not `Core` directly) *(since 0.1.5)*. |
...
**Test-loop feedback** *(since 0.1.5)* — the `.nt` file on disk is
...
`ProxyOptions.Redaction` *(since 0.1.5)* threads a custom
...
- `NARRATIVETRACE_OUTPUT` (`true`/`false`/`1`/`0`, default `true` — on by
  default *(since 0.1.5)*; only an explicit `false`/`0` opts out)
