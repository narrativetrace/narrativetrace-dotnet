...
| `<output-dir>/manifest.json` | No | Regenerated every run; its top-level `run` object (`id`, `name` — the run's own three-word phrase) names *this execution*, not a scenario, so it changes on every run even when nothing else does *(since 0.1.4, unreleased)*. |
